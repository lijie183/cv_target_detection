...
def train_model(data_dir, output_dir, config_path, num_epochs=10, batch_size=2, lr=0.005, 
                backbone='resnet50', use_albumentations=True):
    """训练Faster R-CNN模型"""
    # 创建输出目录
    os.makedirs(output_dir, exist_ok=True)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=4, collate_fn=utils.collate_fn, pin_memory=True)
    ...
    for epoch in range(num_epochs):
        ...
        results['train_loss'].append(train_loss.avg)
        ...
        val_loss = sum(loss for loss in val_metrics.values()) / len(val_metrics)
        val_map = val_metrics.coco_eval['bbox'].stats[0]  # mAP @ IoU=0.5:0.95
        ...
        # 记录到TensorBoard
        tb_writer.add_scalar('Loss/train', train_loss.avg, epoch)
        ...
        # 打印进度
        logger.info(f"周期 {epoch+1}/{num_epochs}: 训练损失: {train_loss.avg:.4f}, "
