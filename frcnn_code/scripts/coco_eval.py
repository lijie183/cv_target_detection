...
def loadRes(self, resFile):
    ...
    # Check result type
    if isinstance(resFile, str):  # 修改这里，str替换torch._six.string_classes
        anns = json.load(open(resFile))
    elif isinstance(resFile, list):
        anns = resFile
    else:
        raise Exception("results in not valid type")

    assert isinstance(anns, list), "results is not a list"
